...
stochastic penal colony. It demonstrates that mostly punitive usage of \V{AI} is
...
After all, when \V{AI} detects and intercepts violative content before it is
...
the very people they are intended to protect, notably us \V{LGBT}
...
supported by a majority of \V{US} respondents in at least one survey. Remarkably,
...
content moderation based on the reported fraction of videos flagged by \V{AI}.
...
omitted about half of \V{US} users, as discovered 1.5~years later, which called
...
chief \V{AI} scientist---because they kept upstaging Meta's noticeably incomplete
...
been misleading customers, investors, and regulators alike~\anon{\cite{Grimm2022a}}.
...
political polarization in Washington \V{DC}, it seems highly unlikely that
Congress will be able to agree on legislation even remotely comparable in
ambition and scope to the \V{EU}'s \V{DSA}. Still, that doesn't mean Congress
...
At the same time, it's not that hard to draft legislation that codifies a few
...
\begin{enumerate}
    ...
    \item Provide granular logs to researchers approved by National Science
        Foundation in the \V{US} or local equivalent. \label{itm:law:research}
\end{enumerate}
...
\V{DSA}~\cite{EuropeanParliamentAndCouncil2022}. It provides a convenient and
...
equivalent provisions of the \V{DSA}~\cite{Vermeulen2022}. The latter relies on
...
Having to ramp up all these functions before February 2024, most \V{EU} members
are turning to existing regulatory agencies to also take on \V{DSA} monitoring and
...
same firms for the \V{GDPR} has been decidedly ho-hum, with several
...
Probably the biggest weakness of the \V{DSA} is its dependence on national
regulators. Another shortcoming, at least from the perspective of this
independent investigator, is the \V{DSA}'s mandate that researchers be
affiliated with a research institution. At the same time, the \V{DSA}'s
...
experience with national regulators insufficiently enforcing \V{GDPR} did result
in the European Commission taking on \V{DSA} oversight functions in addition to
...
research on social media by more teams. It's a nice touch that the \V{DSA}
...
Also except that Douek's Harvard Law Review article~\cite{Douek2022} raises a
...
with governments, e.g., when it comes to \V{CSAM}, relying on outside
...
reviews and appeals to become more equitable. Meanwhile, the \V{DSA}'s risk
...
\S\ref{sec:trusting-twitter}), Meta not disclosing \V{NCMEC} reports or unique
pieces when reporting \V{CSAM} (see \S\ref{sec:census-validation}), and most
...
content, and for \V{CSAM}, reports to \V{NCMEC}. Since they all capture a different
...
has not devoted all of his not insubstantial powers as \V{US} president to razing
...
reused the name for ``the Blitz,'' Germany's \V{WWII} bombing campaign against
...
Still, I view devolution as a clear win for users because it significantly
...
When it comes to \V{AI}, previous work on dataset, model, and system cards already
...
That's exactly the reason why we should always reject ``disclosing
...
organization and \V{AI} deployments. Or as Cory Doctorow succinctly put it in the
...
Since there is no meaningful regulation requiring transparency about \V{AI}-based
interventions and so many basic legal questions about training data and \V{AI}
...
the day, \V{AI} certainly is artificial but it has little to do with human
intelligence. It fundamentally is software, software that runs on computers.
Like all other software humans have created so far, \V{AI} is buggy. The kinds of
...
the most advanced \V{AI} techniques~\cite{MetzSchmidt2023}, far more than a single
...
outlines of a new intellectual property regime for \V{AI}. The primary goals for
the new \V{IP} regime are to hold those who are pushing the state-of-the-art in
\V{AI} accountable for their deployments and to ensure that everyone benefits from
\V{AI}. As before, I am particularly concerned with transparency not because it
suffices, but because it is critical for facilitating sound policy decisions in
the future. The new \V{AI} \V{IP} or \V{AIP} regime is inspired by (1)~OpenAI's
charter, (2)~\V{US} copyright provisions for works created by the government, and
...
``lesser'' \V{AI}s as well. The firm states that \V{AI} should be ``used for the
...
organization's) word for it. \V{US} copyright law points to just the right means
for enforcing that: Almost all works created by the government immediately enter
the public domain and are reusable by everyone. So \V{AIP} does the same for
\emph{all} \V{AI} systems including machine learning models and enters them into
...
for \V{AI} also nicely accounts for recent models incorporating much material of
...
\V{AI} actually proprietary, most private investment into \V{AI} would likely wither
away. So \V{AIP} takes a cue from patent law, which grants a temporary, legal
monopoly to inventors---as long as they fully disclose the invention upfront.
Hence, \V{AIP} grants people and organizations a temporary, legal monopoly on an
\V{AI} if they follow certain rules---more on them in a moment. Given recent rapid
progress in \V{AI}, setting the monopoly length to the same 20 years as for patents
seems excessive. Instead, I am proposing a duration of three to five years. To
gain this temporary protected status, an \V{AI} operator files an intent notice
including a preliminary system card with the national \V{AIP} office before
...
that an almost identical \V{AI} has already been registered. By collecting the
actual artifacts, the \V{AIP} website becomes a national if not global
clearinghouse for \V{AI} and ensures that, with a couple of years lag, everyone
gains access to the same advanced technology. While the \V{AI} has protected
...
that gives \V{AI} researchers direct access to the \V{AI} for experimentation.
...
\V{NSF} in \S\ref{sec:regulating:moderation}, the \V{AIP} agency performs basic
...
\V{AI}, really is little more than the recombination of proven elements from
...
open and when Google and Microsoft still had ethical \V{AI}
...
The proposed intellectual property regime, \V{AIP}, targets \V{AI} only and
...
elements from existing \V{IP} law. It also balances the expansive application of
...
process. In contrast, applications for \V{AIP}'s protected status do not require
...
simpler review, which largely can be automated. All that makes \V{AIP} a rather
