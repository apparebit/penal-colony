...
That organization also facilitated integration of criteria, such as the role of
trusted flaggers or visibility reduction, that do not fit as neatly into common
conceptions of content moderation focused on individuals more or less
misbehaving. The resulting hierarchy follows, with emoji serving as column
headers in the aforementioned table.

% \hfill pushes the nested enumerations onto the next line!

\begin{description}
\item[Violative content or behavior]\hfill
    \begin{enumerate}
    ...
    \end{enumerate}
\item[The entity reporting violative content or behavior] including reported vs
    acted-upon reports\hfill
    ...
    \end{enumerate}
\item[Actions on content and users]\hfill
    \begin{enumerate}[resume]
...
\item[Due process protections]\hfill
    \begin{enumerate}[resume]
        ...
        \item \emo{eye}~External oversight: policy formulation, conflict
            resolution, auditing \label{itm:oversight}
    \end{enumerate}
\item[Global differences and impact]\hfill
    \begin{enumerate}[resume]
        ...
        \item \emo{globe-africa-europe}~Resources and results for different
            countries and cultures \label{itm:countries}
    \end{enumerate}
\end{description}
...
For the seventeen criteria listed above, the table contains a checkmark if a
platform includes \emph{some} statistics for that criterion in its transparency
reports. For criteria~\ref{itm:prevalence, itm:reach, itm:government,
itm:flaggers, itm:users, itm:algorithms, itm:moderators, itm:labels,
itm:visibility, itm:removal, itm:accounts, itm:appeals, itm:countries}, the data
should further differentiate between specific prohibitions. Though in practice,
most platforms' government-initiated reports and country-specific breakdowns do
not meet that standard. For criteria~\ref{criterion:moderators}, the data should
include cover topics no platform bothered to address. They include counts of
in-house vs external moderators, the mental health impact on human moderators,
which can be devastating, as well as the resources spent for each country.
...
\begin{tabular}{L@{\;\;}CcCc@{\;}c@{\;}c@{\;}c@{\quad}c@{\;}c@{\;}c@{\;}c@{\;}c@{\quad}c@{\;}c@{\;}c@{\;}c@{\quad}c@{\;}c@{\;}c@{\quad}c@{\;}cC}

\multicolumn{2}{C}{\textbf{Platform}} & \textbf{Account} & \textbf{Latest}
& \multicolumn{18}{c}{\textbf{With \emph{Some} Coverage in Transparency Report\T}} & \\
...
& & & & \emo{eu} & \emo{eu} & \emo{eu} & \emo{eu} & & \emo{eu} & \emo{eu} & \emo{eu} & \emo{eu} &
...
severity of this sanction regime only leads to significant disenfranchisement
from digital platforms, at times with traumatic consequences~\cite{Hill2022},
and turns social media into highly effective boosters of the stochastic penal
colony.
...
more reports than \NCMEC. In a dedicated section, Snap explicitly mentions
\CSAM\ and \NCMEC\ reporting but then labels the statistic ``Total Account
Deletions.'' For H1 2021, it also discloses the fraction of ``accounts enforced
globally,'' which amounts to a 14\% larger quantity. But even with that larger
quantity, its number of account deletions still is 35\% smaller than the number
of reports received by \NCMEC. Given Snap's stated zero-tolerance policy, those
numbers should match. Though it is possible that Snap doesn't consistently
follow through with account closure. By contrast, Twitter discloses (presumably
